...
export default function PurchaseOrdersPage() {
  ...
  // synthetic invoices). Client-side Blob + anchor download instead (same
  // pattern as the Requisitions page's CSV export) -- downloads this as a
  // standalone .html file; the browser's own Print -> Save as PDF turns it
  // into an actual PDF if that's specifically needed. (A client-side PDF
  // library was tried here twice -- html2pdf.js, whose CJS-only dist build
  // Next.js's webpack couldn't turn into a valid dynamic-import chunk; then
  // jsPDF + html2canvas directly -- but per explicit instruction this was
  // reverted back to the plain .html download as the simpler, more
  // reliable option. Manual Print -> Save as PDF is an accepted tradeoff.)
